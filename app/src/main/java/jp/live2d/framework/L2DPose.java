...
public final class L2DPose {
    ...
    public final void updateParam(ALive2DModel model) {
        ...
        for (L2DPartsParam[] partsGroup : partsGroups) {
    ...
    private void initParam(ALive2DModel model) {
        ...
        for (L2DPartsParam[] partsGroup : partsGroups) {
            for (L2DPartsParam aPartsGroup : partsGroup) {
                aPartsGroup.initIndex(model);

                int partsIndex = aPartsGroup.partsIndex;
                int paramIndex = aPartsGroup.paramIndex;
                if (partsIndex < 0)
                    continue; // 存在しないパーツです
                ...
                if (aPartsGroup.link == null)
                    continue;
                for (int k = 0; k < aPartsGroup.link.size(); k++) {
                    aPartsGroup.link.get(k).initIndex(model);
                }
            }
        }
    }
     ...
     * @param deltaTimeSec
     */
    private void normalizePartsOpacityGroup(ALive2DModel model, L2DPartsParam partsGroup[], float deltaTimeSec) {
        int visibleParts = -1;
     ...
     * @param partsGroup
     */
    private void copyOpacityOtherParts(ALive2DModel model, L2DPartsParam partsGroup[]) {
...
final class L2DPartsParam {
     ...
     * @param model
     */
    public final void initIndex(ALive2DModel model) {
